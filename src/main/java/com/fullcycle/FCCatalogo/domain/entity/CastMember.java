...
public class CastMember extends BaseEntity {
    ...
    public CastMember( CastMemberType type,String name){
        super.generateUUID();
        this.setType(type);
        this.setName(name);
    }

    public CastMember(UUID id, CastMemberType type,String name){
        super.setId(id);
        this.setType(type);
        this.setName(name);
    }
    ...
    public void setType(CastMemberType type) {
        ...
        if(!CastMemberType.valueOf(type)) throw new IllegalArgumentException("name is marked enum but is not a valid enum");
        this.type = type;
    }
